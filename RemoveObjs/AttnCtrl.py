...
def save_as_pkl(var, file_name: str, folder_path: str = "pkls"):
    """
    将一个 Python 变量保存到一个 pickle 文件中。如果指定的文件名已存在，则自动在文件名后添加序号以避免覆盖。

    函数首先检查目标文件夹是否存在，如果不存在则创建。然后检查文件名是否已被占用，
    若是，则在原有文件名后加上序号（例如 filename(1).pkl）以创建新的文件名。最后，
    使用 pickle 模块将变量序列化并保存到文件中。

    arguments:
        var: 需要保存的 Python 变量。
        file_name (str): 保存文件的名称，不需要包含 ".pkl" 扩展名。
        folder_path (str, 可选): 保存文件的目录，默认为 "pkls"。
...
class RemoveObjsAttentionControlEdit(AttentionStore, abc.ABC):
    def step_callback(self, x_t):
        return x_t
    ...
    def forward(
        ...
        super(RemoveObjsAttentionControlEdit, self).forward(
            ...
            if attn.shape[-1] == 77:
                ...
                if (self.cur_step + 1) in CrossAttnEditSteps:
                    ...
                    if isResetMask:
                        ...
                        if mask is not None:
                            for Word in CrossAttnEditIndex:
                                to_zero = attn[1][:, :, Word]
                                # [8, h_w^2]
    ...
    def __init__(
        ...
        num_steps: int,
        self_replace_steps,
        cross_replace_steps: Union[float, Tuple[float, float]] = None,  # 新增
    ):
        super(RemoveObjsAttentionControlEdit, self).__init__()
